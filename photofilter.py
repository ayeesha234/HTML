...
def apply_filter(frame, filter_type):
    if filter_type == 'GRAYSCALE':
        ...
        sepia_frame = np.clip(sepia_frame, 0, 255)  # Ensure valid pixel values
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Take picture if thumb touches index
            if abs(thumb_x - index_x) < 30 and abs(thumb_y - index_y) < 30:
                if current_time - last_action_time > debounce_time:
                    print("📸 Picture clicked!")  # You can add saving code here
                    last_action_time = current_time

            # Change filter if thumb touches any other finger
